...
function liquid_attributes_post( $attributes ) {
	...
	if ( ! empty( $post ) ) {
		...
		if( !class_exists( 'ReduxFramework' ) || !class_exists( 'Liquid_Addons' ) ) {
			$attributes['class'] = join( ' ', get_post_class( array( $attributes['class'], 'lqd-lp lqd-lp-style-22 lqd-lp-title-34 pos-rel' ) ) );
		}
		else {
...
function liquid_attributes_entry_title( $attributes ) {
	...
	if( !class_exists( 'ReduxFramework' ) || !class_exists( 'Liquid_Addons' ) ) {
		$attributes['class']    = isset( $attributes['class'] ) ? $attributes['class'] . ' lqd-lp-title mt-2 mb-3 h5 lh-125' : '';
	}
	else {
...
function liquid_attributes_entry_published( $attributes ) {
	...
	$attributes['title']    = get_the_time( esc_html_x( 'l, F j, Y, g:i a', 'post time format', 'hub' ) );
...
function liquid_attributes_entry_summary( $attributes ) {
	...
	if( !class_exists( 'ReduxFramework' ) || !class_exists( 'Liquid_Addons' ) ) {
		$attributes['class'] = 'entry-summary lqd-lp-excerpt mt-3 mb-3';
	}
	else {
...
function liquid_post_thumbnail() {
	...
	echo '<div class="lqd-lp-img overflow-hidden border-radius-5 mt-4 mb-5">';
		the_post_thumbnail();
	echo '</div>';
}
...
function liquid_get_post_terms( $args ) {
	...
	$defaults = array(
		...
		'sep'        => esc_html_x( ', ', 'taxonomy terms separator', 'hub' )
	);
	$args = wp_parse_args( $args, $defaults );

	$classnames = 'lqd-lp-cat reset-ul inline-nav pos-rel z-index-2';
...
function liquid_post_title_description() {
	...
	elseif ( is_search() )
		$title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'hub' ), get_search_query() );
	...
	elseif ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
		$title = get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'hub' ) );

	elseif ( get_query_var( 'minute' ) )
		$title = sprintf( esc_html__( 'Minute %s', 'hub' ), get_the_time( esc_html_x( 'i', 'minute archives time format', 'hub' ) ) );

	elseif ( get_query_var( 'hour' ) )
		$title = get_the_time( esc_html_x( 'g a', 'hour archives time format', 'hub' ) );

	elseif ( is_day() )
		$title = get_the_date( esc_html_x( 'F j, Y', 'daily archives date format', 'hub' ) );

	elseif ( get_query_var( 'w' ) )
		$title = sprintf( esc_html__( 'Week %1$s of %2$s', 'hub' ), get_the_time( esc_html_x( 'W', 'weekly archives date format', 'hub' ) ), get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'hub' ) ) );

	elseif ( is_month() )
		$title = single_month_title( ' ', false );

	elseif ( is_year() )
		$title = get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'hub' ) );

	elseif ( is_archive() )
		$title = hesc_html__( 'Archives', 'hub' );
