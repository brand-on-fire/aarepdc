...
class Import_Eventbrite_Events_Pro{
	...
	public static function instance() {
		if( ! isset( self::$instance ) && ! (self::$instance instanceof Import_Eventbrite_Events_Pro ) ) {
			...
			if( iee_free_plugin_activated() ){
				...
				// Elementor Init
				add_action( 'init', array( self::$instance, 'iee_Elementor_init_actions' ) );
			}else{
	 ...
	 * @since 1.0.0
	 */
	public function __clone() { _doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'import-eventbrite-events-pro' ), '1.7.0' ); }
	 ...
	 * @since 1.0.0
	 */
	public function __wakeup() { _doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'import-eventbrite-events-pro' ), '1.7.0' ); }
	...
	/**
	 * Include Custom Elementor class for Eventbrite Events.
	 *
	 * @since 1.5.0
	 * @return void
	 */
	function iee_Elementor_init_actions() {
		// Require elementor class
		require_once IEEPRO_PLUGIN_DIR . 'includes/page-builder/class-iee-elementor-widget.php';
	}
	...
	function iee_free_activatation_notice() {
		...
		<div class="error">
			<p>
				<?php
				printf( __( '<strong>Import Eventbrite Events Pro</strong> requires free version <a href="%s" target="_blank">Import Eventbrite Events</a>. Please <a href="%s" class="thickbox open-plugin-details-modal">Install</a> & Activate it. <a href="%s" target="_blank">More info.</a>', 'import-eventbrite-events-pro' ), 'https://wordpress.org/plugins/import-eventbrite-events/', esc_url( admin_url( "plugin-install.php?tab=plugin-information&plugin=import-eventbrite-events&TB_iframe=true&width=600&height=550" ) ), 'http://docs.xylusthemes.com/docs/import-eventbrite-events-plugin/plugin-installation-pro/' );
				?>
			</p>
		</div>
		<?php
	...
	private function setup_constants() {
		...
		// Google maps API Key
		if( ! defined( 'IEEPRO_GM_APIKEY' ) ){
			define( 'IEEPRO_GM_APIKEY', 'AIzaSyCmDwwCg3dGcorCUf0q59VYtyZ74xUxT1Q' );
		}
...
// Get Import_Eventbrite_Events_Pro Running.
