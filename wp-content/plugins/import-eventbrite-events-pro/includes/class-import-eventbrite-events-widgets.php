...
class Import_Eventbrite_Events_Pro_Upcoming_Widget extends WP_Widget {
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['direct_link'] = $new_instance['direct_link'] ? 1 : 0;
		return $instance;
	}
	...
	public function form( $instance ) {
		...
		$this->render_input_field( 'direct_link', $direct_link, __( 'Direct Event Source link', 'import-eventbrite-events-pro' ), 'checkbox' );
	}
	...
	public function xt_render_upcoming_events( $args, $instance ){
		...
		$posts_per_page = 10; 
		$display_style = 'style1';
		$is_display_image = $is_display_location = $is_display_enddate = $is_display_desc = $is_new_window = $is_direct_link = false;
		...
		$accent_color = isset( $options['accent_color'] ) ? $options['accent_color'] : '#039ED7';
		/* Start the Loop */
		ob_start();
		?>
			...
			if( $fb_events->have_posts() ):
				while ( $fb_events->have_posts() ) : $fb_events->the_post();
					...
					if($time_format == '12hours' ){
						$start_time			= date_i18n( 'h:i a', $event_start_str );
						$end_time           = date_i18n( 'h:i a', $event_end_str );
						$d_start_time       = date_i18n('F j (h:i a) ', $event_start_str );
						$d_end_time        	= date_i18n('F j (h:i a) ', $event_end_str );
					}elseif($time_format == '24hours' ){
						$start_time         = date_i18n( 'G:i', $event_start_str );
						$end_time           = date_i18n( 'G:i', $event_end_str );
						$d_start_time       = date_i18n('F j (G:i) ', $event_start_str );
						$d_end_time         = date_i18n('F j (G:i) ', $event_end_str );
					}else{
						$start_time         = date_i18n( get_option( 'time_format' ), $event_start_str );
						$end_time           = date_i18n( get_option( 'time_format' ), $event_end_str );
						$d_start_time       = date_i18n('F j ('. get_option( 'time_format' ). ')', $event_start_str );
						$d_end_time        	= date_i18n('F j ('. get_option( 'time_format' ). ')', $event_end_str );
					}
					
					$event_date = $d_start_time;
					if( $is_display_enddate ){
						if( $event_start_str != $event_end_str ){
							if( date_i18n('Y-m-d', $event_start_str ) == date_i18n('Y-m-d', $event_end_str ) ){
								$event_date = date_i18n('F j', $event_start_str ) . ' (' . $start_time . ' - ' . $end_time. ')';
							} else {
								$event_date = $d_start_time . ' - ' . $d_end_time;
							}
						}
					}
