...
$filter_enable_counter = ['filter_enable_counter'];
...
echo '<div class="lqd-pf-grid ' . $this->get_id() . '">';
	...
	if( 'masonry-creative' === $style ) {
		...
		if( 'yes' === $atts['show_filter'] ) {
			...
			printf( '<div id="%1$s" class="lqd-pf-row row d-flex flex-wrap %1$s" data-liquid-masonry="true" data-masonry-options=\'{ "layoutMode": "packery"' . $opt_counter . ' }\' %2$s>', $this->grid_id, $this->get_options() );
		}
	}
